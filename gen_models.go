...
func generateModel(ctx context.Context, db *sql.Tx, patterns []string) error {
	const qry = `SELECT table_name, column_name, data_type, data_length, NVL(data_precision, 0), NVL(data_scale, 0), nullable
      FROM user_tab_cols WHERE table_name LIKE 'T\_DIGIKAR\_%' ESCAPE '\' 
      ORDER BY 1, 2`
...
`); err != nil {
	...
	{
		const qry = `SELECT B.index_name, B.table_name, B.column_name FROM user_ind_columns B, user_indexes A 
            WHERE B.table_name = A.table_name AND B.index_name = A.index_name AND A.uniqueness = 'UNIQUE' AND 
                  A.table_name LIKE 'T\_DIGIKAR\_%' ESCAPE '\' 
