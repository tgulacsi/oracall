...
`)
	buf2.WriteString(`
// File generated by oracall. DO NOT EDIT.
...
import (
	...
	for _, nm := range names {
		...
		if err = x.WriteProtobufMessageType(ctx, &buf); err != nil {
			t.Fatal(nm, err)
		}

		if x.ProtoMessageName() == "" {
			continue
		}
		if err = x.WriteOraToFrom(ctx, &buf2); err != nil {
			t.Fatal(nm, err)
		}
	}
	...
	os.WriteFile("testdata/funcs_ora.go", buf2.Bytes(), 0664)
...
plugins:
  ...
  - local: ["go", "tool", "protoc-gen-go-grpc"]
    ...
    opt:
      - paths=source_relative
  - local: ["go", "tool", "protoc-gen-go-json"]
    out: .
    opt:
      - paths=source_relative
